...
<c:choose>
	<c:when
		test="${empty sessionScope.student && not empty sessionScope.teacher}">
		<c:set var="id" value="${sessionScope.teacher}"></c:set>
		<c:set var="loginClass" value="t"></c:set>
	</c:when>
	<c:when
		test="${not empty sessionScope.student && empty sessionScope.teacher}">
		<c:set var="id" value="${sessionScope.student}"></c:set>
		<c:set var="loginClass" value="s"></c:set>
	</c:when>
	<c:when
		test="${empty sessionScope.student && empty sessionScope.teacher && not empty sessionScope.admin}">
...
<c:set var="buyConfirm" value="${requestScope.buyConfirm}"></c:set>
<div>
	<hr>
</div>
...
<div class="container">
	<div class="lec">
		<div class="row">
			<div class="col-8">
				<div class="lec-img">
					<img src="${pageContext.request.contextPath}/images/lecmain.jpg" />
				</div>
				...
				<div class="lec-content1">
					<div class="size" id="section1">
						...
						<div class="small-font">${lessonDetail.les_description}</div>
					</div>
					...
					<div class="size" id="section5">
						...
						<div>
							...
							폐지 등으로 교습을 계속할 수 없는 경우에는 학습자로부터 받은<br> 교습비를 반환하는 등 학습자를 보호하기
							위하여 필요한 조치를 하여야 한다.<br> <br> 1. 레슨을 제공할 수 없거나, 레슨 장소를
							제공할 수 없게 된 날 : 이미 납부한 레슨비 등을 일한 계산한 금액 환불<br> <br> 2.
							레슨기간이 1개월 이내의 경우<br> - 레슨 시작전 : 이미 납부한 레슨비 전액 환불<br> -
							총 레슨 시간의 1/3 경과전 : 이미 납부한 레슨비의 2/3에 해당액 환불<br> - 총 레슨 시간의
							1/2 경과전 : 이미 납부한 레슨비용의 1/2에 해당액 환불<br> - 총 레슨시간의 1/2 경과후 :
							반환하지 않음<br> <br> 3.레슨 기간이 1개월을 초과하는 경우<br> - 레슨
							시작전 : 이미 납부한 레슨비 전액 환불<br> - 레슨 시작후 : 반환사유가 발생한 당해 월의 반환 대상
							레슨비(레슨비 징수기간이 1개월 이내인 경우에 따라 산출된 수강료를 말한다)와 나머지 월의 레슨비 전액을 합산한 금액
							환불<br> <br> * 총 레슨 시간의 레슨비 징수기간 중의 총레슨시간을 말하며, 반환 금액의
							산정은 반환 사유가 발생한 날까지 경과 된 레슨시간을 기준으로 함<br>
						</div>
					</div>
				...
				<div class="container">
					<div class="lec-content2">
						<div class="lec-about">
								...
								<div class="">
									...
									<div class="test test_button d-inline-block">
										<a id="li" href="#" class="button button_light"> <span
											class="heart">&hearts;</span> <span class="text_like">Like
												x<span id="likeList1">${likeList}</span>
										</span> <span class="text_liked">Liked x<span id="likeList2">${likeList}</span></span>
										</a>
									</div>
							...
							<hr>
							<a id="buyLes" href="#" class="btn btn-block btn-primary btn-default"
								style="background-color: #2e21df">구매하기(${lesson.les_price}원)</a>
						</div>
						<!-- 장소 -->
						<div class="lec-add">
							...
							<div class="d-flex justify-content-center pb-3">
								<div class="image_outer_container">
									<div class="image_inner_container">
										<img src="${pageContext.request.contextPath}/images/member/Guest.png">
									</div>
								</div>
							</div>
...
var no = "${lesson.les_no}";
	...
	$(function() {
		if (loginClass!='s') {
			if (buyConfirm > 0) {
				$('#buyLes').css("backgroundColor", "gray");
				$('#buyLes').off("click");
				$('#buyLes').html("결제하신 상품");
			} 
		} else {
		...
		$.ajax({
			url : '${pageContext.request.contextPath}/mainfront?sid=findLikeList&no=' + no + '&stu_id=test1',
			type : 'get',
			dataType : 'text',
		...
		$(document)
				.on(
						...
						function(e) {
							e.preventDefault();
							if (loginClass == "s") {
								$(this).closest('.button').toggleClass(
										'selected');

								if ($(this).closest('.button').hasClass(
										"selected") === true) {
									$
											.ajax({
												url : 'mainfront?sid=likelistinsert&no='
														+ no + '&stu_id=' + id,
												type : 'get',
												dataType : "text",
												success : function(result) {
									...
									$
											.ajax({
												url : 'mainfront?sid=likelistdelete&no='
														+ no + '&stu_id=' + id,
												type : 'get',
												dataType : "text",
												success : function(result) {
